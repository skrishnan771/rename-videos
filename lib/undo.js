...
function runUndo(targetPath) {
  ...
  // A syntactically valid but structurally wrong log (hand-edited, truncated,
  // or from a future format) would otherwise blow up on log.entries.length
  // and surface as a bare "Unexpected error"
  if (!log || !Array.isArray(log.entries)) {
    console.error(c.red(`✖  Undo log is malformed (no entries list): ${logPath}`));
    console.error(c.gray('   Delete it and re-run, or restore a valid log.'));
    process.exit(1);
  }
