...
const {
  SMALL_WORDS, RELEASE_GROUP_RE, RESOLUTION_RE,
  STOP_RE, STOP_RE_STRICT, LANG_RE, LANG_RE_STRICT,
} = require('./constants');
...
/**
 * Returns the index where "technical junk" (a stop word or language tag)
 * first begins in str, or str.length if none is found. Checks all four
 * word lists — the case-insensitive safe lists plus their case-sensitive
 * "strict" counterparts (words that double as ordinary English words, only
 * treated as junk in exact-uppercase scene-tag form — see constants.js).
 */
function firstJunkIndex(str) {
  return Math.min(
    firstMatchIndex(str, STOP_RE),
    firstMatchIndex(str, STOP_RE_STRICT),
    firstMatchIndex(str, LANG_RE),
    firstMatchIndex(str, LANG_RE_STRICT)
  );
}
...
function stripReleaseGroup(str) {
  ...
  // RELEASE_GROUP_RE already constrains the capture to [A-Z][A-Z0-9]{1,14},
  // so no second all-caps test is needed — it could never fail.
  return m ? str.slice(0, m.index).trim() : str;
}
